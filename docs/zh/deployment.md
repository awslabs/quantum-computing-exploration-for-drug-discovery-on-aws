...
6. 填写策略名称。本次部署填入：`qradd-quicksight-service-role-policy`。
...
14. 填写**角色名称**。本次部署填入： `qradd-quicksight-service-role`。
...
2. 记录对应您的电子邮件的**用户名**。
    ...
    !!! Caution "注意"
        ...
        如果您的登录IAM用户不是注册QuickSight账户的用户，您需要登录[QuickSight](https://quicksight.aws.amazon.com/)，并在页面中填写您的电子邮件地址。
