...
class BuildMolGraph():
    ...
    def build_graph(self):
        ...
        for index, row in self.df_bonds.iterrows():
            logging.debug("atom 1 {} atom 2 {}".format(row['atom1'], row['atom2']))
            add_node(nodes_list, row['atom1'])
            add_node(nodes_list, row['atom2'])
            add_edge(edges_list, (row['atom1'],row['atom2']), self.non_ar_bonds, True, row['bond_type'])
    ...
    def build_rb_data(self):
        ...
        #             print(pts)
                    ...
                    i = i + 1
                # update bc score
                rb_data[current_rb_name]['bc_num'] = (self.bc[rb[0]] + self.bc[rb[1]])/2
    ...
    def build_ris_data(self):
        ...
        def update_ris_data(ris_data, rb_data, candidate_pair):
            ...
            for atom in candidate_pair:
                side = judge_side_by_metrics(rb_data[ris_data[bond_name]['metrics']], atom)
        ...
        sor_ris_data = {k:v for k,v in sorted(ris_data.items(), key=lambda ris: (ris[1]['rb_count_num'], -ris[1]['avg_bc_num']))}
        return sor_ris_data